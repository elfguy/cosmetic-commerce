...
export const productOpsStatuses: ProductOpsStatus[] = [
  ...
  },
  {
    ...
    detailStatus: 'V3 다운로드 이미지 상세 구성 완료',
    assetStatus: '상세 이미지 버전 관리 중',
    claimRisk: 'medium',
    nextAction: 'V3 대표 5장과 상세 9장을 최종 검수 후 쿠팡 업로드용으로 선별',
...
export const commandTasks = [
  ...
  {
    label: '채택 후보',
    title: '아쿠아로션 V3 다운로드 이미지 최종 검수',
    product: '아쿠아 로션',
    href: '/products/aqua-lotion/',
    state: '검토',
  },
  {
    label: '세트 구성',
  ...
  'aqua-lotion': [
    {
      ...
      status: 'candidate',
      date: '2026-06-01',
      summary: 'ChatGPT 웹/GPT Images에서 생성 후 다운로드한 V4 상세 10장 후보입니다. 대표 이미지는 기존 요청대로 V2 대표 이미지를 유지합니다.',
      mainImages: Array.from({ length: 6 }, (_, i) => '/coupang/images/aqua-lotion/versions/v4-gpt-web/representative/' + String(i + 1).padStart(2, '0') + '.png'),
      detailImages: Array.from({ length: 10 }, (_, i) => '/coupang/images/aqua-lotion/versions/v4-gpt-web/detail/' + String(i + 1).padStart(2, '0') + '.png'),
      links: [
        { label: 'V4 보기', href: '/marketing/aqua-lotion-version-compare/' },
        { label: 'V4 컨택시트', href: '/coupang/images/aqua-lotion/versions/v4-gpt-web/v4-gpt-web-contact-sheet.png' },
      ],
      metrics: [
        { label: '대표', value: '6' },
        { label: '상세', value: '10' },
