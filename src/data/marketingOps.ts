...
export const productOpsStatuses: ProductOpsStatus[] = [
  ...
  },
  {
    ...
    detailStatus: '원본 상세 14컷 + V1 첫 컷 후보 비교 중',
    assetStatus: 'V1 후보 검수 중 — 쿠팡 미반영',
    claimRisk: 'high',
    nextAction: '원본 첫 화면과 V1 첫 컷을 나란히 보고 기능성 근거/전환력 비교',
...
export const productVersions: Record<string, ProductVersion[]> = {
  ...
  'whitening-cream': [
    ...
    },
    {
      ...
      status: 'candidate',
      date: '2026-06-07',
      summary: '화이트닝 톤 케어 크림 V1 후보입니다. 대표 01~05번 신규 컷과 상세 01~15번 신규 컷 전체를 분리했습니다. 대표 01은 상품 only + 신선 마크, 03은 사람 사용감 컷입니다.',
      mainImages: Array.from({ length: 5 }, (_, i) => '/coupang/images/whitening-cream/versions/v1/representative/' + String(i + 1).padStart(2, '0') + '.png'),
      detailImages: Array.from({ length: 15 }, (_, i) => '/coupang/images/whitening-cream/versions/v1/detail/' + String(i + 1).padStart(2, '0') + '.png'),
      links: [
      ...
      metrics: [
        { label: '대표', value: '5 신규' },
        { label: '상세', value: '15 신규' },
        { label: '교체', value: '대표 01~05 / 상세 01~15' },
      ],
    },
  ],
};
