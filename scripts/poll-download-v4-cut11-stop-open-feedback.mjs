...
const targetUrl = (await fs.readFile(path.join(outDir, 'prompts/11-stop-open-feedback-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/11-stop-open-feedback-before-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 120; attempt++) {
  ...
  const candidates = data.imgs.map(x => ({ ...x, id: getId(x.src) }))
    .filter(x => x.w >= 700 && x.h >= 1000 && !beforeIds.has(x.id) && !String(x.alt || '').startsWith('cut11-stopopen-'));
  ...
  if (uniq.length && !data.busy) {
    ...
    const rawFile = path.join(outDir, 'raw/11-stop-open-feedback-gpt.png');
    const finalFile = path.join(outDir, 'detail/11.png');
    const stamp = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
    const backupFile = path.join(outDir, `rejected/11-before-stop-open-feedback-${stamp}.png`);
    ...
    await fs.writeFile(path.join(outDir, 'prompts/11-stop-open-feedback-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${rawFile}\nfinal=${finalFile}\nbackup=${backupFile}\n`);
