...
rows = [
    ...
    ("화장품제조업자 및\n화장품책임판매업자", "(주)유어스킨"),
    ("제조국", "대한민국"),
    ("[화장품법]에 따라 기재.표시해야 하는 모든 성분", "정제수, 글리세린, 카프릴릭/카프릭트라이글리세라이드, 프로판다이올, 글리세릴스테아레이트에스이, 다이프로필렌글라이콜, 마카다미아씨오일, 글리세릴스테아레이트에스이, 다이프로필렌글라이콜, 해바라기씨오일, 하이드록시아세토페논, 솔비탄스테아레이트, 카보머, 카프릴릴글라이콜, 세테아릴알코올, 시어버터, 소듐하이알루로네이트(2,050ppm), 알지닌, 해수, 부틸렌글라이콜, 로즈힙열매오일, 1,2-헥산다이올, 토코페롤, 병풀추출물, 효모/겨우살이발효추출물, 효모/띠뿌리발효추출물, 락토바실러스/콩발효추출물, 다이포타슘글리시리제이트, 밀싹추출물, 브로콜리추출물, 양배추추출물, 자주개자리추출물, 하이드록시프로필트라이모늄하이알루로네이트, 무씨추출물, 유채추출물, 소듐아세틸레이티드하이알루로네이트(0.5ppm), 하이드롤라이즈드하이알루로닉애씨드(0.5ppm), 하이알루로닉애씨드(0.125ppm), 소듐하이알루로네이트크로스폴리머(0.05ppm), 하이드롤라이즈드소듐하이알루로네이트(0.05ppm), 포타슘하이알루로네이트(0.01ppm)"),
