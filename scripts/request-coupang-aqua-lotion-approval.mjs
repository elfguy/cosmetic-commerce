...
function auth(method,path,query=''){const d=new Date().toISOString().slice(2,19).replace(/[-:]/g,'')+'Z'; const sig=crypto.createHmac('sha256',process.env.COUPANG_SECRET_KEY).update(d+method+path+query).digest('hex'); return `CEA algorithm=HmacSHA256, access-key=${process.env.COUPANG_ACCESS_KEY}, signed-date=${d}, signature=${sig}`;}
...
if(!execute){ console.log(JSON.stringify({ok:true,mode:'dry-run',approvalPath,note:'No approval request sent'},null,2)); process.exit(0); }
const r=await req('PUT', approvalPath);
const outDir=path.join(root,'data/coupang/update/aqua-lotion/approval-'+new Date().toISOString().replace(/[-:]/g,'').replace(/\.\d{3}Z$/,'Z'));
