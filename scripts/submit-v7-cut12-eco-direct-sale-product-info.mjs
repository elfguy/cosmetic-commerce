...
const uploadFiles=[
 'cut12-original-eco-direct-sale.png',
 'cut12-tone-ref-10.png',
 'cut12-tone-ref-11.png',
 'cut12-product-ref-front.png',
 'cut12-product-ref-full.png',
].map(f=>path.join(refDir,f));
const prompt=await fs.readFile(path.join(outDir,'prompts/12-eco-direct-sale-product-info-submitted.txt'),'utf8');
...
await fs.writeFile(path.join(outDir,'prompts/12-eco-direct-sale-product-info-before-ids.json'),JSON.stringify(before.map(getId),null,2));
...
if(after < before.length + 5){
 await page.screenshot({path:path.join(root,'tmp-v7-cut12-upload-failed.png'),fullPage:true});
 console.error('upload_maybe_failed');
 process.exit(4);
}
...
await page.waitForTimeout(15000);
await fs.writeFile(path.join(outDir,'prompts/12-eco-direct-sale-product-info-chat-url.txt'),page.url()+'\n');
await page.screenshot({path:path.join(root,'tmp-v7-cut12-submitted.png'),fullPage:true});
