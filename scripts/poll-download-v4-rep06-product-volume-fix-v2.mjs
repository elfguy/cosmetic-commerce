...
const targetUrl = (await fs.readFile(path.join(promptDir, '06-product-volume-fix-v2-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(promptDir, '06-product-volume-fix-v2-before-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 150; attempt++) {
  ...
  if (uniq.length && !data.busy) {
    ...
    const raw = path.join(rawDir, '06-product-volume-fix-v2-gpt.png');
    await fs.writeFile(raw, buf);
    const current = path.join(repDir, '06.png');
    const ts = new Date().toISOString().replace(/[-:TZ.]/g, '').slice(0,14);
    try { await fs.copyFile(current, path.join(rejectedDir, `06-before-product-volume-fix-v2-${ts}.png`)); } catch {}
    await sharp(buf).resize(1254, 1254, { fit:'cover', position:'center' }).png({ compressionLevel:9 }).toFile(current);
    await fs.writeFile(path.join(promptDir, '06-product-volume-fix-v2-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${raw}\nfinal=${current}\n`);
