...
const productSlug = 'hyaluronic-toner-v5-representative';
const versionRoot = path.join(root, 'public/coupang/images/hyaluronic-acid-toner/versions/v5');
...
const headChecks = await Promise.all([imageUrls.representation, ...imageUrls.additionalImages].map(httpHead));
...
const preservedContents = structuredClone(targetItem.contents || []);
targetItem.images = [
...
targetItem.contents = preservedContents;
...
if (!execute) {
  console.log(JSON.stringify({
    ...
    prepared: {
      version: 'v5',
      representativeImageCount: targetItem.images.length,
      preservedContentGroups: targetItem.contents.length,
      preservedDetailImageCount: targetItem.contents.flatMap((content) => content.contentDetails || []).length,
    },
