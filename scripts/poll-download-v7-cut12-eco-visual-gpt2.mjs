...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/12-eco-visual-gpt2-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/12-eco-visual-gpt2-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=80;attempt++){
 ...
 const candidates=data.imgs.map(x=>({...x,id:getId(x.src)})).filter(x=>x.w>=700&&x.h>=500&&!beforeIds.has(x.id));
 ...
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/12-eco-visual-gpt2.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/12-eco-visual-gpt2-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n${img.alt||''}\n`);
