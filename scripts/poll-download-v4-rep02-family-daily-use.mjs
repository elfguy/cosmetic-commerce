...
const targetUrl = (await fs.readFile(path.join(promptDir, '02-gpt-family-daily-use-chat-url.txt'), 'utf8')).trim();
...
for (let attempt=1; attempt<=120; attempt++) {
  ...
  if (uniq.length && !data.busy) {
    ...
    const raw = path.join(rawDir, '02-family-daily-use-gpt.png');
    await fs.writeFile(raw, buf);
    const current = path.join(repDir, '02.png');
    try {
      const ts = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
      await fs.copyFile(current, path.join(rejectedDir, `02-banner-replaced-by-family-daily-use-${ts}.png`));
    } catch {}
    await sharp(buf).resize(1254, 1254, { fit:'cover', position:'center' }).png({ compressionLevel: 9 }).toFile(current);
    await fs.writeFile(path.join(promptDir, '02-gpt-family-daily-use-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${raw}\nfinal=${current}\n`);
