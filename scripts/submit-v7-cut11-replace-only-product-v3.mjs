...
const uploadDir=path.join(root,'tmp/cut11-replace-only-upload-v3');
const uploadFiles=(await fs.readdir(uploadDir)).filter(f=>f.endsWith('.png')).map(f=>path.join(uploadDir,f));
const prompt=await fs.readFile(path.join(outDir,'prompts/11-replace-only-product-v3-submitted.txt'),'utf8');
...
await fs.writeFile(path.join(outDir,'prompts/11-replace-only-product-v3-before-ids.json'),JSON.stringify(before.map(getId),null,2));
...
if(after < before.length + 3){ await page.screenshot({path:path.join(root,'tmp-v7-cut11-replace-only-product-v3-upload-failed.png'),fullPage:true}); console.error('upload_maybe_failed'); process.exit(4); }
...
await page.waitForTimeout(15000);
await fs.writeFile(path.join(outDir,'prompts/11-replace-only-product-v3-chat-url.txt'),page.url()+'\n');
await page.screenshot({path:path.join(root,'tmp-v7-cut11-replace-only-product-v3-submitted.png'),fullPage:true});
