...
const targetUrl = (await fs.readFile(path.join(outDir, 'prompts/11-pump-detail-images-url-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/11-pump-detail-images-url-before-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 120; attempt++) {
  ...
  const candidates = data.imgs.map(x => ({ ...x, id: getId(x.src) }))
    .filter(x => x.w >= 700 && x.h >= 1000 && !beforeIds.has(x.id) && !String(x.alt || '').startsWith('cut11-pumpdetail-'));
  ...
  if (uniq.length && !data.busy) {
    ...
    const rawFile = path.join(outDir, 'raw/11-pump-detail-images-url-gpt.png');
    const finalFile = path.join(outDir, 'detail/11.png');
    const stamp = new Date().toISOString().replace(/[-:T]/g,'').slice(0,14);
    const backupFile = path.join(outDir, `rejected/11-before-pump-detail-${stamp}.png`);
    ...
    await fs.writeFile(path.join(outDir, 'prompts/11-pump-detail-images-url-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${rawFile}\nfinal=${finalFile}\nbackup=${backupFile}\n`);
