...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/11-refroles-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/11-refroles-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=100;attempt++){
 ...
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/11-refroles-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/11-refroles-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
