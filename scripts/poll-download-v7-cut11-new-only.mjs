...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/11-chat-url.txt'),'utf8')).trim();
const exclude=new Set([
 ...JSON.parse(await fs.readFile(path.join(outDir,'prompts/11-before-ids.json'),'utf8')),
 'file_00000000f67c7206945793c82597fc33'
]);
...
for(let attempt=1;attempt<=120;attempt++){
 ...
 const data=await page.evaluate(()=>({busy:!!document.querySelector('[aria-label*="중지"], [aria-label*="Stop"], button[data-testid="stop-button"]'), text:document.body.innerText.slice(-1000), imgs:Array.from(document.images).map(img=>({alt:img.alt,src:img.currentSrc||img.src,w:img.naturalWidth,h:img.naturalHeight,rect:(()=>{const r=img.getBoundingClientRect();return{x:r.x,y:r.y,w:r.width,h:r.height};})()})).filter(x=>x.src.includes('backend-api/estuary/content'))}));
 const candidates=data.imgs.map(x=>({...x,id:getId(x.src)})).filter(x=>x.w>=700&&x.h>=1000&&!exclude.has(x.id)&&!String(x.alt||'').startsWith('cut11-'));
 ...
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/11-how-to-use-open-pump-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/11-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
  console.log('saved',rawFile,img.id,buf.length,`${img.w}x${img.h}`);
  process.exit(0);
 }
}
console.error('no new image found'); process.exit(2);
