...
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/10-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=60;attempt++){
 ...
 if(candidates.length){
  ...
  const rawFile=path.join(outDir,'raw/10-fresh-manufacture-date-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/10-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
  console.log('saved',rawFile,img.id,buf.length,`${img.w}x${img.h}`);
  process.exit(0);
 }
}
console.error('no cut10 image found'); process.exit(2);
