...
const targetUrl = (await fs.readFile(path.join(promptDir, '01-gpt-product-main-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(promptDir, '01-gpt-product-main-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=120; attempt++) {
  ...
  for (const x of data.imgs.map(x => ({...x, id:getId(x.src)})).filter(x => x.w>=900 && x.h>=900 && !beforeIds.has(x.id) && !String(x.alt||'').startsWith('rep01-gpt-main'))) {
  ...
  if (uniq.length && !data.busy) {
    ...
    const raw = path.join(rawDir, '01-gpt-product-main.png');
    await fs.writeFile(raw, buf);
    const current = path.join(repDir, '01.png');
    try {
      const ts = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
      await fs.copyFile(current, path.join(rejectedDir, `01-local-composite-replaced-by-gpt-${ts}.png`));
    } catch {}
    const final = current;
    await sharp(buf).resize(1254, 1254, { fit:'cover', position:'center' }).png({ compressionLevel: 9 }).toFile(final);
    await fs.writeFile(path.join(promptDir, '01-gpt-product-main-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${raw}\nfinal=${final}\n`);
