...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/11-replace-only-product-v3-chat-url.txt'),'utf8')).trim();
const knownBad=new Set(['file_00000000f67c7206945793c82597fc33','file_00000000813c7209b09f557203733d8c']);
...
for(let attempt=1;attempt<=120;attempt++){
 ...
 console.log('attempt',attempt,'busy',data.busy,'cand',candidates.map(x=>({id:x.id,w:x.w,h:x.h,alt:x.alt,y:x.rect.y})), 'tail', data.text.slice(-180).replace(/\n/g,' | '));
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/11-replace-only-product-v3-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/11-replace-only-product-v3-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n${img.alt||''}\n`);
