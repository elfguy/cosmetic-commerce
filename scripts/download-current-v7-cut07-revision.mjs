...
const targetUrl='https://chatgpt.com/c/6a1e6a79-88f0-83ab-826a-9ff823103972';
...
const gen=imgs.filter(x=>x.w>=900&&x.h>=1500&&!String(x.alt||'').startsWith('cut07-')).at(-1);
...
await fs.writeFile(path.join(outDir,'prompts/07-revision-result.txt'), `${targetUrl}\n${gen.src}\n${new URL(gen.src).searchParams.get('id')}\n${buf.length}\n${gen.w}x${gen.h}\n`);
