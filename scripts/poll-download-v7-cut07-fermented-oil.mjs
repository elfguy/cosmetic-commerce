...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/07-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isCut07 = data.text.includes('발효 오일 블렌딩') || data.text.includes('cut07-original-fermented-oil') || data.text.includes('식물 유래 오일 블렌딩') || data.text.includes('호모/겨우살이');
    if(!isCut07) continue;
    targetPages.push(p);
  ...
  if(candidates.length){
    ...
    const rawFile=path.join(outDir,'raw/07-fermented-oil-gpt.png');
    await fs.writeFile(rawFile,buf);
    await fs.writeFile(path.join(outDir,'prompts/07-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut07 image found');
process.exit(2);
