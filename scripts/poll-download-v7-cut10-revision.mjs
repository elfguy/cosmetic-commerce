...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/10-revision-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/10-revision-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=60;attempt++){
 ...
 if(candidates.length){
  ...
  const rawFile=path.join(outDir,'raw/10-fresh-manufacture-date-remove-v7-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/10-revision-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
  console.log('saved',rawFile,img.id,buf.length,`${img.w}x${img.h}`);
  process.exit(0);
 }
}
console.error('no cut10 revision image found'); process.exit(2);
