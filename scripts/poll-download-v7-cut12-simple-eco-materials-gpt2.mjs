...
const key='12-simple-eco-materials-gpt2';
const targetUrl=(await fs.readFile(path.join(outDir,`prompts/${key}-chat-url.txt`),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,`prompts/${key}-before-ids.json`),'utf8')));
...
for(let attempt=1;attempt<=80;attempt++){
 ...
 const candidates=data.imgs.map(x=>({...x,id:getId(x.src)})).filter(x=>x.w>=700&&x.h>=700&&!beforeIds.has(x.id)&&!/img_a0d7bfea|reference/i.test(String(x.alt||'')));
 console.log('attempt',attempt,'busy',data.busy,'candidates',candidates.map(x=>({id:x.id,w:x.w,h:x.h,alt:x.alt,y:x.rect.y})), 'tail', data.text.slice(-120).replace(/\n/g,' | '));
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/12-simple-eco-materials-gpt2.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,`prompts/${key}-result.txt`),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n${img.alt||''}\n`);
