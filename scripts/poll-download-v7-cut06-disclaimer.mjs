...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/06-disclaimer-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isCut06 = data.text.includes('상기내용은 원료적 특성') || data.text.includes('쿠팡 원본처럼 특허증') || data.text.includes('cut06-ref-coupang-original-patent') || data.text.includes('원료 조성물입니다');
    if(!isCut06) continue;
    targetPage=p;
  ...
  if(candidates.length){
    ...
    const rawFile=path.join(outDir,'raw/06-coupang-patent-disclaimer-gpt.png');
    await fs.writeFile(rawFile,buf);
    await fs.writeFile(path.join(outDir,'prompts/06-disclaimer-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut06 disclaimer image found');
process.exit(2);
