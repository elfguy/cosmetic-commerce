...
for (const n of targets) {
  ...
  try { keepUrls.push((await fs.readFile(path.join(promptDir, `${nn}-product-swap-main01-chat-url.txt`), 'utf8')).trim()); } catch {}
}
const browser = await chromium.connectOverCDP('http://127.0.0.1:9222');
const ctx = browser.contexts()[0] || await browser.newContext();
await cleanupStaleImageAgentTabs(ctx, { keepUrls, maxTabs: 3 });

for (const n of targets) {
  const nn = String(n).padStart(2, '0');
  const targetUrl = (await fs.readFile(path.join(promptDir, `${nn}-product-swap-main01-chat-url.txt`), 'utf8')).trim();
  const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(promptDir, `${nn}-product-swap-main01-before-ids.json`), 'utf8')));
  ...
  for (let attempt=1; attempt<=150; attempt++) {
    ...
    for (const x of data.imgs.map(x => ({...x, id:getId(x.src)})).filter(x => x.w >= 700 && x.h >= 900 && !beforeIds.has(x.id))) {
    ...
    if (uniq.length && !data.busy) {
      ...
      const raw = path.join(rawDir, `${nn}-product-swap-main01-gpt.png`);
      await fs.writeFile(raw, buf);
      const current = path.join(detailDir, `${nn}.png`);
      try {
        const ts = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
        await fs.copyFile(current, path.join(rejectedDir, `${nn}-before-product-swap-main01-${ts}.png`));
      } catch {}
      await sharp(buf).resize(780, 1360, { fit:'cover', position:'center' }).png({ compressionLevel:9 }).toFile(current);
      await fs.writeFile(path.join(promptDir, `${nn}-product-swap-main01-result.txt`), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${raw}\nfinal=${current}\n`);
      console.log('saved', { n, current, id:img.id, bytes:buf.length, dims:`${img.w}x${img.h}` });
      saved=true;
      break;
    }
  }
  if (!saved) throw new Error('no generated image saved for detail '+nn);
}
console.log('saved detail product swaps');
