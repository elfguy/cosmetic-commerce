...
const refs = [
  [path.join(outDir, 'detail/07.png'), path.join(outDir, 'reference/cut07-current-needs-ferment-ingredients.png')],
  [path.join(root, 'public/coupang/images/aqua-lotion/versions/original/detail/07.png'), path.join(outDir, 'reference/cut07-original-ferment-ingredient-structure.png')],
  [path.join(root, 'tmp-v7-01-06-contact.png'), path.join(outDir, 'reference/cut07-v7-tone-board-01-06-ferment-revision.png')],
  [path.join(outDir, 'detail/06.png'), path.join(outDir, 'reference/cut07-tone-ref-06.png')],
];
...
[수정 이유]
- 현재 cut07-current-needs-ferment-ingredients.png는 톤은 좋지만, 발효성분이 무엇인지 정보가 약하고 가운데가 비어 보인다.
- 이번 수정은 기존 톤과 하단 오일 블렌딩 느낌은 유지하면서, 상단/중앙에 발효성분 3종의 원료 이미지와 이름을 더 명확하게 넣는 것이다.

[가장 중요한 요구사항]
- 발효 성분 3종이 이미지 안에서 명확히 보여야 한다.
- 원본 cut07-original-ferment-ingredient-structure.png처럼 발효성분 원료 사진/원료 일러스트가 함께 보이게 한다.
- 가운데 빈 공간을 채우기 위해 중앙에 “3중 발효 컨디셔닝 콤플렉스” 느낌의 원형 다이어그램 또는 연결 그래픽을 넣는다.
- 단, Point 4 같은 번호 라벨은 절대 넣지 않는다.
...
[중앙 발효성분 영역 — 반드시 선명하게]
3개의 유리 카드 또는 원형 접시를 크게 배치하고, 각 카드 안에 원료 이미지를 넣는다.
1) 효모/겨우살이추출물
   - 겨우살이 잎/가지/열매를 떠올리는 식물 원료 이미지 + 발효 버블
2) 효모/띠뿌리발효추출물
   - 뿌리 원료 이미지 + 발효 버블
3) 락토바실러스/콩발효추출물
   - 콩/대두 원료 이미지 + 발효 버블
...
[가운데 보강]
- 상단 3개 발효성분과 하단 오일 블렌딩 사이가 비어 보이지 않도록, 중앙에 부드러운 반투명 원형 다이어그램을 넣는다.
- 중앙 문구 후보:
  3중 발효 컨디셔닝 콤플렉스
  피부를 편안하게 감싸는 수분 컨디셔닝
- 너무 복잡한 긴 문장은 넣지 말고, 정보 밀도를 자연스럽게 채운다.
...
[톤앤매너]
- 기존 V7 톤 유지: 화이트 + 연아쿠아 + 민트그린 + 부드러운 골드 오일 포인트.
- 현재 07의 맑은 수분감과 고급스러운 오일 드롭은 유지.
- 상단 발효성분 이미지는 원본처럼 정보성 있게, 하지만 V7처럼 세련되게.
- 제품 병/패키지는 넣지 않는다.
...
- 원료명을 틀리지 말 것. 특히 '효모/겨우살이추출물', '효모/띠뿌리발효추출물', '락토바실러스/콩발효추출물'을 명확히 표시한다.`;

await fs.writeFile(path.join(outDir, 'prompts/07-ferment-ingredients-revision-submitted.txt'), prompt);
...
await fs.writeFile(path.join(outDir, 'prompts/07-ferment-revision-before-ids.json'), JSON.stringify(before.map(getId), null, 2));
...
await page.screenshot({ path: path.join(root, 'tmp-v7-cut07-ferment-revision-submitted.png'), fullPage: true });
await fs.writeFile(path.join(outDir, 'prompts/07-ferment-revision-chat-url.txt'), page.url() + '\n');
