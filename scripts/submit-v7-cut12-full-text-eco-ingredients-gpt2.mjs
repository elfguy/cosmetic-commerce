...
const prompt=await fs.readFile(path.join(outDir,'prompts/12-full-text-eco-ingredients-gpt2-submitted.txt'),'utf8');
const uploadFiles=[
 '/Users/elfguy/.hermes/image_cache/img_a0d7bfea667f.jpeg',
 path.join(outDir,'detail/12.png'),
 path.join(outDir,'detail/10.png'),
...
await fs.writeFile(path.join(outDir,'prompts/12-full-text-eco-ingredients-gpt2-before-ids.json'),JSON.stringify(before.map(getId),null,2));
...
if(await input.count()){
  ...
  console.error('upload input not found'); process.exit(4);
}
...
await fs.writeFile(path.join(outDir,'prompts/12-full-text-eco-ingredients-gpt2-chat-url.txt'),page.url()+'\n');
await page.screenshot({path:path.join(root,'tmp-v7-cut12-full-text-gpt2-submitted.png'),fullPage:true});
