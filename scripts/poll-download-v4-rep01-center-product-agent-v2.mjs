...
const targetUrl = (await fs.readFile(path.join(promptDir, '01-center-product-agent-v2-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(promptDir, '01-center-product-agent-v2-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=120; attempt++) {
  ...
  if (uniq.length && !data.busy) {
    ...
    const raw = path.join(rawDir, `01-center-product-agent-v2-${stamp}.png`);
    const norm = path.join(rawDir, `01-center-product-agent-v2-${stamp}-1254.png`);
    await fs.writeFile(raw, buf);
    await sharp(buf).resize(1254, 1254, { fit:'cover', position:'center' }).png({ compressionLevel: 9 }).toFile(norm);
    await fs.writeFile(path.join(promptDir, '01-center-product-agent-v2-result.json'), JSON.stringify({ targetUrl, src: img.src, id: img.id, natural: `${img.w}x${img.h}`, bytes: buf.length, raw, normalized: norm }, null, 2));
