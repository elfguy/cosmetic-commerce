...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/10-regenerate-v2-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/10-regenerate-v2-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=90;attempt++){
 ...
 if(candidates.length){
  ...
  const rawFile=path.join(outDir,'raw/10-fresh-manufacture-date-regenerate-v2-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/10-regenerate-v2-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
