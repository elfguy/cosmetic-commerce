...
const refs = [
  ...
  path.join(base, 'refs/aqua-v4-detail-04.png'),
...
await page.screenshot({ path: path.join(root, 'tmp-toner-routine-rev4-before-send.png'), fullPage: true });
...
const disabled = await send.evaluate(el => el.disabled || el.getAttribute('aria-disabled') === 'true').catch(()=>null);
console.log('send disabled?', disabled);
await send.click({ timeout: 15000 });
await page.waitForTimeout(10000);
await fs.writeFile(path.join(base, 'raw', `${key}-submit-log.json`), JSON.stringify({key,startUrl:'https://chatgpt.com/images/',afterUrl:page.url(),title:await page.title(),refs,submittedAt:new Date().toISOString()}, null, 2));
