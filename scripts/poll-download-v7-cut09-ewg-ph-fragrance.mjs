...
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/09-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=90;attempt++){
 ...
 for(const p of pages){
  ...
  const isCut09=data.text.includes('cut09-v3-06-ewg-test-info')||data.text.includes('약산성 pH의 내용은 V2')||data.text.includes('전성분 100% EWG')||data.text.includes('pH5.0 ~ pH6.5');
  if(!isCut09) continue;
  targetPages.push(p);
 ...
 if(candidates.length){
  const generated=candidates.filter(x=>!String(x.alt||'').startsWith('cut09-'));
  ...
  await fs.writeFile(path.join(outDir,'prompts/09-result.txt'),`${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
  console.log('saved',rawFile,img.id,buf.length,`${img.w}x${img.h}`,img.pageUrl);
  process.exit(0);
 }
}
console.error('no cut09 image found'); process.exit(2);
