...
const targetUrl = (await fs.readFile(path.join(outDir, 'prompts/10-realistic-white-product-images-url-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/10-realistic-white-product-images-url-before-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 120; attempt++) {
  ...
  const candidates = data.imgs
    ...
    .filter(x => x.w >= 700 && x.h >= 1000 && !beforeIds.has(x.id) && !String(x.alt || '').startsWith('cut10-realwhite-'));
  const uniq = [];
  const seen = new Set();
  for (const c of candidates) if (!seen.has(c.id)) { seen.add(c.id); uniq.push(c); }
  console.log('attempt', attempt, 'busy', data.busy, 'candidates', uniq.map(x => ({ id: x.id, w: x.w, h: x.h, alt: x.alt, y: x.rect.y })));
  if (uniq.length && !data.busy) {
    ...
    const rawFile = path.join(outDir, 'raw/10-realistic-white-product-images-url-gpt.png');
    const finalFile = path.join(outDir, 'detail/10.png');
    const stamp = new Date().toISOString().replace(/[-:T]/g,'').slice(0,14);
    const backupFile = path.join(outDir, `rejected/10-before-realistic-white-${stamp}.png`);
    ...
    await fs.writeFile(path.join(outDir, 'prompts/10-realistic-white-product-images-url-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${rawFile}\nfinal=${finalFile}\nbackup=${backupFile}\n`);
    console.log('saved', { rawFile, finalFile, backupFile, id: img.id, bytes: buf.length, size: `${img.w}x${img.h}` });
    process.exit(0);
  }
}
console.error('no finished image found');
process.exit(2);
