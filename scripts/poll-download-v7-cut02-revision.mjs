...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/02-revision-before-estuary-ids.json'), 'utf8')));
const chatUrl = 'https://chatgpt.com/c/6a1e22cf-13ec-83ab-83be-a2682abd2f4d';
...
for (let attempt = 1; attempt <= 60; attempt++) {
  ...
  if (candidates.length) {
    ...
    const rawFile = path.join(outDir, 'raw/02-family-daily-revision-no-logo-no-product-gpt.png');
    await fs.writeFile(rawFile, buf);
    await fs.writeFile(path.join(outDir, 'prompts/02-revision-result.txt'), `${page.url()}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
