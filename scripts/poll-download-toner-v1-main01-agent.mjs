...
const key = '01-representative-main-agent';
const targetUrl = 'https://chatgpt.com/c/6a2243ad-a5d4-83a6-9aa4-0be2a776affc';
...
for (let i=0;i<90;i++) {
  ...
  const candidates = data.imgs.map(x => ({...x, id:getId(x.src)}))
    .filter(x => x.w >= 900 && x.h >= 900 && !beforeIds.has(x.id));
  ...
  if (candidates.length && !data.busy) {
    // Prefer near-square generated image, not uploaded refs.
    const generated = candidates.filter(c => Math.abs(c.w/c.h - 1) < 0.25 && !String(c.alt||'').includes('히알루론산토너'));
...
if (!chosen) { await page.screenshot({path:path.join(root,'tmp-toner-main01-agent-poll-failed.png'), fullPage:true}); throw new Error('no generated candidate'); }
