...
const uploadFiles=[
 'cut12-v2v3-01-structure-product-info-ingredients.png',
 'cut12-v2v3-02-product-info-ingredients-source.png',
 'cut12-v2v3-03-official-seller-reference.png',
 'cut12-v2v3-04-v7-tone-ref-10.png',
 'cut12-v2v3-05-v7-tone-ref-11.png',
 'cut12-v2v3-06-current-remove-product-image.png',
].map(f=>path.join(refDir,f));
const prompt=await fs.readFile(path.join(outDir,'prompts/12-v2v3-info-ingredients-no-product-submitted.txt'),'utf8');
...
await fs.writeFile(path.join(outDir,'prompts/12-v2v3-info-ingredients-no-product-before-ids.json'),JSON.stringify(before.map(getId),null,2));
...
if(after < before.length + 6){ await page.screenshot({path:path.join(root,'tmp-v7-cut12-v2v3-upload-failed.png'),fullPage:true}); console.error('upload_maybe_failed'); process.exit(4); }
...
await page.waitForTimeout(15000);
await fs.writeFile(path.join(outDir,'prompts/12-v2v3-info-ingredients-no-product-chat-url.txt'),page.url()+'\n');
await page.screenshot({path:path.join(root,'tmp-v7-cut12-v2v3-submitted.png'),fullPage:true});
