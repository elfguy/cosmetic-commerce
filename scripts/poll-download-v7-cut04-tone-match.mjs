...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/04-tone-match-before-estuary-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isCut04 = data.text.includes('04번 이미지를 다시 생성') || data.text.includes('톤앤매너') || data.text.includes('cut04-tone-ref') || data.text.includes('강한 딥블루');
    if (!isCut04) continue;
    targetPage = p;
  ...
  if (candidates.length) {
    const generated = candidates.filter(x=>!String(x.alt||'').startsWith('cut04-'));
    ...
    await fs.writeFile(path.join(outDir,'prompts/04-tone-match-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut04 tone-match image found');
process.exit(2);
