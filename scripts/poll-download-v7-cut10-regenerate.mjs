...
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/10-regenerate-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=90;attempt++){
 ...
 const candidates=data.imgs.map(x=>({...x,id:getId(x.src)})).filter(x=>x.w>=700&&x.h>=1000&&!beforeIds.has(x.id)&&!String(x.alt||'').startsWith('cut10-'));
 ...
 if(candidates.length){
  ...
  const rawFile=path.join(outDir,'raw/10-fresh-manufacture-date-regenerate-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/10-regenerate-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
  console.log('saved',rawFile,img.id,buf.length,`${img.w}x${img.h}`);
  process.exit(0);
 }
}
console.error('no cut10 regenerated image found'); process.exit(2);
