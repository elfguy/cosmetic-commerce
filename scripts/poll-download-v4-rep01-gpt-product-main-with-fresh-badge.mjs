...
const targetUrl = (await fs.readFile(path.join(promptDir, '01-gpt-product-main-with-fresh-badge-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(promptDir, '01-gpt-product-main-with-fresh-badge-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=120; attempt++) {
  ...
  console.log('poll', attempt, 'busy', data.busy, 'candidates', uniq.map(x => ({ id:x.id, w:x.w, h:x.h, alt:x.alt })));
  if (uniq.length && !data.busy) {
    ...
    const raw = path.join(rawDir, '01-gpt-product-main-with-fresh-badge.png');
    await fs.writeFile(raw, buf);
    const current = path.join(repDir, '01.png');
    try {
      const ts = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
      await fs.copyFile(current, path.join(rejectedDir, `01-gpt-no-badge-replaced-by-fresh-badge-${ts}.png`));
    } catch {}
    await sharp(buf).resize(1254, 1254, { fit:'cover', position:'center' }).png({ compressionLevel: 9 }).toFile(current);
    await fs.writeFile(path.join(promptDir, '01-gpt-product-main-with-fresh-badge-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${raw}\nfinal=${current}\n`);
    console.log('saved', current, img.id, buf.length, `${img.w}x${img.h}`);
    process.exit(0);
  }
}
process.exit(2);
