...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/10-v3-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/10-v3-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=90;attempt++){
 ...
 if(candidates.length){
  ...
  const rawFile=path.join(outDir,'raw/10-code-date-no-check-v3-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/10-v3-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
