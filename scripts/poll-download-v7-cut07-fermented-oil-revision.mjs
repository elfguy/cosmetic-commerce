...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/07-revision-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isCut07 = data.text.includes('감사 보호') || data.text.includes('cut07-current-good-but-typo') || data.text.includes('한국어 오타') || data.text.includes('끈적임 없이');
    if(!isCut07) continue;
    targetPages.push(p);
  ...
  if(candidates.length){
    ...
    const rawFile=path.join(outDir,'raw/07-fermented-oil-revision-gpt.png');
    await fs.writeFile(rawFile,buf);
    await fs.writeFile(path.join(outDir,'prompts/07-revision-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut07 revision image found');
process.exit(2);
