...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/06-v2-style-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isThis = data.text.includes('기존 이미지 방식이 가장 좋다') || data.text.includes('cut06-v2-style-base') || data.text.includes('특허 인증서를 넣되') || data.text.includes('6가지 새싹 복합 성분');
    if(!isThis) continue;
    targetPages.push(p);
  ...
  if(candidates.length){
    ...
    await fs.writeFile(path.join(outDir,'prompts/06-v2-style-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut06 v2-style image found');
process.exit(2);
