...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/04-hyaluronic-before-estuary-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isCut04 = data.text.includes('8종 히알루론산 함유') || data.text.includes('한 방울로 겉부터 속까지') || data.text.includes('cut04-ref-user-v1-hyaluronic-expert') || data.text.includes('04번 이미지를 다시 생성');
    if (!isCut04) continue;
    targetPage = p;
  ...
  if (candidates.length) {
    ...
    const rawFile=path.join(outDir,'raw/04-hyaluronic-expert-v1like-gpt.png');
    await fs.writeFile(rawFile,buf);
    await fs.writeFile(path.join(outDir,'prompts/04-hyaluronic-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut04 hyaluronic image found');
process.exit(2);
