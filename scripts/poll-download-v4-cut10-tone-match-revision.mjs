...
const targetUrl = (await fs.readFile(path.join(outDir, 'prompts/10-tone-match-revision-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/10-tone-match-revision-before-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 120; attempt++) {
  ...
  const candidates = data.imgs
    ...
    .filter(x => x.w >= 700 && x.h >= 1000 && !beforeIds.has(x.id) && !String(x.alt || '').startsWith('cut10-tonefix-'));
  ...
  if (candidates.length && !data.busy) {
    ...
    const rawFile = path.join(outDir, 'raw/10-tone-match-revision-gpt.png');
    const finalFile = path.join(outDir, 'detail/10.png');
    const backupFile = path.join(outDir, `rejected/10-before-tone-match-${new Date().toISOString().replace(/[-:T]/g,'').slice(0,14)}.png`);
    await fs.copyFile(finalFile, backupFile).catch(() => {});
    await fs.writeFile(rawFile, buf);
    await sharp(buf)
      .resize(780, 1360, { fit: 'cover', position: 'center' })
      .png({ compressionLevel: 9 })
      .toFile(finalFile);
    await fs.writeFile(path.join(outDir, 'prompts/10-tone-match-revision-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${rawFile}\nfinal=${finalFile}\nbackup=${backupFile}\n`);
