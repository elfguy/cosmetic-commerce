...
const base = path.join(root, 'public/coupang/images/aqua-lotion/versions/v4');
const detailDir = path.join(base, 'detail');
const rawDir = path.join(base, 'agent-detail-raw');
const promptDir = path.join(base, 'prompts');
const cut = process.env.CUT;
const key = process.env.KEY || `${cut}-gpt-v1`;
if (!cut) throw new Error('CUT env required');
const prompt = await fs.readFile(path.join(promptDir, `${key}-prompt.txt`), 'utf8');
const refs = (process.env.REFS || '').split(',').map(s => s.trim()).filter(Boolean).filter(fssync.existsSync);
...
await page.screenshot({ path: path.join(root, `tmp-aqua-${key}-after-upload.png`), fullPage: false });
console.log('attachedLabels', afterUpload.attachedLabels.filter(x => /img_|detail|aqua|이미지|파일|png|jpeg/i.test(x)).slice(0, 40));

await page.locator('#prompt-textarea').click();
await page.keyboard.insertText(prompt);
await page.waitForTimeout(1000);
await page.screenshot({ path: path.join(root, `tmp-aqua-${key}-before-send.png`), fullPage: true });
...
for (let i=1; i<=120; i++) {
  ...
  console.log('poll', i, 'busy', s.busy, 'candidates', candidates.map(c => ({ id:c.id, w:c.w, h:c.h, alt:c.alt })), 'tail', s.text.slice(-100).replace(/\n/g,' | '));
  if (candidates.length && !s.busy) {
    for (const img of [...candidates].reverse()) {
      ...
      if (refHashes.has(hash)) { console.log('reject identical uploaded ref', {id:img.id, rawBytes:buf.length, alt:img.alt}); continue; }
      const raw = path.join(rawDir, `${key}-agent.png`);
      const candidate = path.join(detailDir, `${key}-candidate.png`);
      await fs.writeFile(raw, buf);
      // keep generated image aspect/size for text readability; convert to PNG only
      await sharp(buf).png({ compressionLevel: 9 }).toFile(candidate);
      const meta = await sharp(candidate).metadata();
      saved = { cut, key, startUrl:'https://chatgpt.com/images/', chatUrl, id:img.id, natural:`${img.w}x${img.h}`, rawBytes:buf.length, raw, candidate, candidateSize:`${meta.width}x${meta.height}`, alt:img.alt, refs, rule:'GPT Images direct output; candidate only; no final overwrite; no local text edits' };
...
if (!saved) throw new Error('no generated candidate');
