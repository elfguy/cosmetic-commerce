...
const promptDir = path.join(base, 'agent-detail-prompts');
...
const defaultRefs = [
  ...
  path.join(root, 'public/coupang/images/hyaluronic-acid-toner/versions/v1/detail/05.png'),
  path.join(root, 'public/coupang-main/whitening-cream/01.png'),
  path.join(root, `public/coupang-detail/9264527939/${cut}.png`),
  ...extraRefs,
];
const refs = [];
...
await page.screenshot({ path: path.join(root, `tmp-whitening-${key}-before-send.png`), fullPage: true });
...
for (let i = 1; i <= 120; i++) {
  ...
  if (candidates.length && !s.busy) {
    ...
    const final = path.join(detailDir, `${cut}.png`);
