...
const key = '02-gpt-product-locked-v4';
...
// Use visible Images upload inputs and verify the original product filename is attached before sending.
...
console.log('attachedOriginal', attachedOriginal, afterUpload.attachedLabels.filter(x => x.includes('whitening') || x.includes('파일') || x.includes('이미지')).slice(0, 20));
...
for (let i = 1; i <= 120; i++) {
  ...
  if (candidates.length && !s.busy) {
    for (const img of [...candidates].reverse()) {
      ...
      saved = { startUrl: 'https://chatgpt.com/images/', chatUrl, id: img.id, natural: `${img.w}x${img.h}`, rawBytes: buf.length, raw, candidate, finalSize: `${meta.width}x${meta.height}`, alt: img.alt, refs, rule: 'GPT Images direct output; saved as candidate only; no final overwrite' };
