...
const targetUrl='https://chatgpt.com/c/6a1e74e1-4e80-83a6-8d94-3625b217cc6e';
...
const gen=imgs.filter(x=>x.w>=900&&x.h>=1500&&!String(x.alt||'').startsWith('cut08-')).at(-1);
...
await fs.writeFile(path.join(outDir,'prompts/08-result.txt'), `${targetUrl}\n${gen.src}\n${new URL(gen.src).searchParams.get('id')}\n${buf.length}\n${gen.w}x${gen.h}\n`);
