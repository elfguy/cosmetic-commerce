...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/11-replace-only-product-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/11-replace-only-product-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=120;attempt++){
 ...
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/11-replace-only-product-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/11-replace-only-product-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
  console.log('saved',rawFile,img.id,buf.length,`${img.w}x${img.h}`);
  process.exit(0);
 }
 if(!data.busy && attempt>12 && /failed|오류|생성할 수/i.test(data.text)){ console.log('possible_error',data.text); process.exit(3); }
}
