...
const jobs=[
  {key:'12-eco-direct-sale-materials-gpt2', raw:'raw/12-eco-direct-sale-materials-gpt2.png'},
  {key:'13-ingredients-transparency-gpt2', raw:'raw/13-ingredients-transparency-gpt2.png'},
];
...
for (const job of jobs) {
 ...
 for(let attempt=1;attempt<=80;attempt++){
  ...
  console.log(job.key,'attempt',attempt,'busy',data.busy,'candidates',candidates.map(x=>({id:x.id,w:x.w,h:x.h,alt:x.alt,y:x.rect.y})), 'tail', data.text.slice(-120).replace(/\n/g,' | '));
  ...
  if(!data.busy && /I can’t|re-upload|다시 업로드|available|failed|오류/i.test(data.text) && attempt>2){
    console.log('response_issue',job.key,data.text); break;
  }
 }
 if(!saved){ console.error('no image found',job.key); process.exit(2); }
}
process.exit(0);
