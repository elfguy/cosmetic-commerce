...
const prompt=await fs.readFile(path.join(outDir,'prompts/12-simple-eco-materials-gpt2-submitted.txt'),'utf8');
const uploadFiles=['/Users/elfguy/.hermes/image_cache/img_a0d7bfea667f.jpeg'];
...
await fs.writeFile(path.join(outDir,'prompts/12-simple-eco-materials-gpt2-before-ids.json'),JSON.stringify(before.map(getId),null,2));
...
if(await input.count()){
  ...
  console.log('upload input not found, continuing without reference');
}
...
await fs.writeFile(path.join(outDir,'prompts/12-simple-eco-materials-gpt2-chat-url.txt'),page.url()+'\n');
await page.screenshot({path:path.join(root,'tmp-v7-cut12-simple-eco-materials-submitted.png'),fullPage:true});
