...
const current=path.join(detailDir,'13.png');
const backup=path.join(rejectedDir,`13-before-official-seller-typo-${stamp}.png`);
if(existsSync(current)) await fs.copyFile(current, backup);
const refs=[
  path.join(detailDir,'12.png'),
  current,
  path.join(root,'public/coupang/images/aqua-lotion/versions/v4/detail/15.png'),
  path.join(root,'public/coupang/images/aqua-lotion/versions/v4/detail/14.png'),
].filter(p=>existsSync(p));
const prompt=`업로드한 참고 이미지들을 기준으로 쿠팡 모바일 상세페이지용 세로 이미지 1장을 다시 만들어주세요.

목표: 유어스킨플러스 히알루론산 토너 상세페이지 13번째 마지막 안내 컷 수정.
중요: 반드시 ChatGPT Images 이미지 생성 결과물 안에 텍스트까지 자연스럽게 포함해 주세요. 배경만 만들지 마세요.

[수정 핵심]
- 업로드한 현재 13번의 전체 톤/레이아웃/카드 구조는 좋습니다.
- 하지만 카드 문구 중 “무료 재판매 제품 주의”는 어색합니다. 반드시 “비공식 재판매 제품 주의”로 바꿔 주세요.
- 제품병, 제품 라벨, 박스, 패키지, 목업은 절대 넣지 마세요.
- 아쿠아 로션 v4 후반부처럼 차분한 흰색/아쿠아/민트 카드형 톤을 유지하세요.
...
[검수 기준]
- “무료 재판매”라는 표현이 남아 있으면 실패입니다.
- 제품병/제품 라벨/박스/패키지가 나오면 실패입니다.
- 컷 번호, POINT/STEP 숫자, V1/V4 같은 버전 표기 금지.
- 한국어가 깨지거나 이상한 글자로 변형되면 실패입니다.`;
await fs.writeFile(path.join(promptDir,'13-official-seller-typo-fix-prompt.txt'),prompt);
await fs.writeFile(path.join(promptDir,'13-official-seller-typo-fix-refs.json'),JSON.stringify({refs,backup},null,2));
...
const chatUrl=page.url(); await fs.writeFile(path.join(promptDir,'13-official-seller-typo-fix-chat-url.txt'),chatUrl+'\n'); console.log('submitted',chatUrl);
for(let i=1;i<=150;i++){
 await page.waitForTimeout(8000); const s=await state(page); const candidates=[]; const seen=new Set();
 for(const img of s.imgs){ if(img.w<900||img.h<900) continue; if(hasKnownId(img.src,beforeIds)) continue; if(refs.some(r=>img.alt===path.basename(r))) continue; const id=primaryId(img.src); if(seen.has(id)) continue; seen.add(id); candidates.push({...img,id}); }
 console.log('poll 13-fix',i,'busy',s.busy,'candidates',candidates.map(c=>({id:c.id,w:c.w,h:c.h,alt:c.alt})));
 if(candidates.length&&!s.busy){ const gen=candidates.filter(c=>c.alt.includes('생성된 이미지')); const img=(gen.length?gen:candidates).at(-1); const b64=await page.evaluate(async(src)=>{const r=await fetch(src,{credentials:'include'}); if(!r.ok) throw new Error('fetch '+r.status); const ab=await r.arrayBuffer(); const bytes=new Uint8Array(ab); let s=''; for(let j=0;j<bytes.length;j+=0x8000)s+=String.fromCharCode(...bytes.subarray(j,j+0x8000)); return btoa(s);},img.src); const buf=Buffer.from(b64,'base64'); const raw=path.join(rawDir,'13-official-seller-typo-fix-agent.png'); const final=path.join(detailDir,'13.png'); await fs.writeFile(raw,buf); await sharp(buf).resize(780,1360,{fit:'cover',position:'center'}).png({compressionLevel:9}).toFile(final); const meta=await sharp(final).metadata(); const result={chatUrl,id:img.id,natural:`${img.w}x${img.h}`,bytes:buf.length,raw,final,finalSize:`${meta.width}x${meta.height}`,backup,refs,alt:img.alt,rule:'direct ChatGPT Images output; normalized only; no local text patch'}; await fs.writeFile(path.join(promptDir,'13-official-seller-typo-fix-result.json'),JSON.stringify(result,null,2)); console.log('saved',JSON.stringify(result)); await page.close().catch(()=>{}); process.exit(0); }
}
