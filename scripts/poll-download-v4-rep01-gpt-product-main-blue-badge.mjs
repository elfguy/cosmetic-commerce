...
const targetUrl = (await fs.readFile(path.join(promptDir, '01-gpt-product-main-blue-badge-chat-url.txt'), 'utf8')).trim();
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(promptDir, '01-gpt-product-main-blue-badge-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=120; attempt++) {
  ...
  if (uniq.length && !data.busy) {
    ...
    const raw = path.join(rawDir, '01-gpt-product-main-blue-badge.png');
    await fs.writeFile(raw, buf);
    const current = path.join(repDir, '01.png');
    try {
      const ts = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
      await fs.copyFile(current, path.join(rejectedDir, `01-green-badge-replaced-by-blue-badge-${ts}.png`));
    } catch {}
    await sharp(buf).resize(1254, 1254, { fit:'cover', position:'center' }).png({ compressionLevel: 9 }).toFile(current);
    await fs.writeFile(path.join(promptDir, '01-gpt-product-main-blue-badge-result.txt'), `${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\nraw=${raw}\nfinal=${current}\n`);
