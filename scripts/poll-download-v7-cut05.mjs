...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/05-before-estuary-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 90; attempt++) {
  ...
  for (const p of pages) {
    ...
    if (!data) continue;
    const isCut05 = data.text.includes('아쿠아로션 상세페이지 05번') || data.text.includes('그래서 피부는 이렇게 달라집니다') || data.text.includes('cut05-ref-skin-result-flow') || data.text.includes('성분이 만드는 피부 변화');
    if (!isCut05) continue;
    targetPage = p;
  ...
  if (candidates.length) {
    const generated = candidates.filter(x => !String(x.alt||'').startsWith('cut05-'));
    ...
    const rawFile = path.join(outDir, 'raw/05-ingredients-to-skin-results-gpt.png');
    await fs.writeFile(rawFile, buf);
    await fs.writeFile(path.join(outDir, 'prompts/05-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut05 image found');
process.exit(2);
