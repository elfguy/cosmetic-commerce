...
const chatUrl = 'https://chatgpt.com/c/6a1e2937-fafc-83a4-b814-b17a585d4f4f';
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/03-target-users-before-ids.json'), 'utf8')));
...
for (let attempt = 1; attempt <= 90; attempt++) {
  ...
  if (candidates.length) {
    ...
    const rawFile = path.join(outDir, 'raw/03-target-users-gpt.png');
    await fs.writeFile(rawFile, buf);
    await fs.writeFile(path.join(outDir, 'prompts/03-target-users-result.txt'), `${page.url()}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`);
    process.exit(0);
  }
}
console.error('no target-users image found');
process.exit(2);
