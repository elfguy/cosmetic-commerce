...
const chatUrl = 'https://chatgpt.com/c/6a1e51a6-01e8-83a6-872e-60708ee3ff61';
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/06-blur-before-ids.json'), 'utf8')));
function getId(src) { try { return new URL(src).searchParams.get('id') || src; } catch { return src; } }
const browser = await chromium.connectOverCDP('http://127.0.0.1:9222');
const ctx = browser.contexts()[0];
let page = ctx.pages().find(p => p.url().includes('/c/6a1e51a6-01e8-83a6-872e-60708ee3ff61')) || await ctx.newPage();
if (!page.url().includes('/c/6a1e51a6')) await page.goto(chatUrl, { waitUntil:'domcontentloaded', timeout:60000 });
...
for (let attempt = 1; attempt <= 90; attempt++) {
  ...
  if (candidates.length) {
    ...
    const rawFile = path.join(outDir, 'raw/06-fresh-bud-patent-blurred-gpt.png');
    await fs.writeFile(rawFile, buf);
    await fs.writeFile(path.join(outDir, 'prompts/06-blur-result.txt'), `${page.url()}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`);
    process.exit(0);
  }
}
console.error('no blur revision image found');
process.exit(2);
