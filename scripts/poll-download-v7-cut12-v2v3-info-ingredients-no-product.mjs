...
const targetUrl=(await fs.readFile(path.join(outDir,'prompts/12-v2v3-info-ingredients-no-product-chat-url.txt'),'utf8')).trim();
const beforeIds=new Set(JSON.parse(await fs.readFile(path.join(outDir,'prompts/12-v2v3-info-ingredients-no-product-before-ids.json'),'utf8')));
...
for(let attempt=1;attempt<=120;attempt++){
 ...
 const candidates=data.imgs.map(x=>({...x,id:getId(x.src)})).filter(x=>x.w>=700&&x.h>=1000&&!beforeIds.has(x.id)&&!/cut12-v2v3|tone-ref|source|reference/i.test(String(x.alt||'')));
 ...
 if(candidates.length && !data.busy){
  ...
  const rawFile=path.join(outDir,'raw/12-v2v3-info-ingredients-no-product-gpt.png');
  await fs.writeFile(rawFile,buf);
  await fs.writeFile(path.join(outDir,'prompts/12-v2v3-info-ingredients-no-product-result.txt'),`${targetUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n${img.alt||''}\n`);
