...
const beforeIds = new Set(JSON.parse(await fs.readFile(path.join(outDir, 'prompts/08-before-ids.json'), 'utf8')));
...
for (let attempt=1; attempt<=90; attempt++) {
  ...
  for (const p of pages) {
    ...
    const isCut08 = data.text.includes('cut08-original-irritation-test') || data.text.includes('피부자극지수 0.00') || data.text.includes('저자극 테스트 완료') || data.text.includes('비자극 제품 인증');
    if(!isCut08) continue;
    targetPages.push(p);
  ...
  if(candidates.length){
    ...
    const rawFile=path.join(outDir,'raw/08-irritation-test-gpt.png');
    await fs.writeFile(rawFile,buf);
    await fs.writeFile(path.join(outDir,'prompts/08-result.txt'), `${img.pageUrl}\n${img.src}\n${img.id}\n${buf.length}\n${img.w}x${img.h}\n`);
    console.log('saved', rawFile, img.id, buf.length, `${img.w}x${img.h}`, img.pageUrl);
    process.exit(0);
  }
}
console.error('no cut08 image found');
process.exit(2);
